...
class Home extends React.Component {
  constructor(p) {
    ...
    this.state= {big_screen:true,loc_data:'', loc_ids:[], annotations:[], setAnnotations:[], newAnnotation:[], setNewAnnotation:[], metadata_form:false, metadata:false, metdata_added:false, uploadFile:false, file_info:{}, annotations_added:false}
  ...
  render() {
    ...
    if (this.props.auth.auth.user.username) {
      return (
        <div ref="myRef">
          <Helmet>
            <title> Audiotation | Process Files </title>
          </Helmet>
          <Header/>
          <Container fluid style={{
            ...
            {this.state.uploadFile ?(
              ...
              <Col xs="12" sm="12" lg="5" style={{fontSize:"1.3rem" , paddingRight:"80px" , textAlign:"right" }} >
               {(window.innerWidth <1400) ?(<span style={{color:"rgb(206 1 57)", paddingRight:"20px"}}>  Please use bigger display screen</span>):(<span style={{fontSize: "1.3rem",
  fontWeight: "500", color: "#888888"}}> Audiotation Files </span>) }
               </Col>
