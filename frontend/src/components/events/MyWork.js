...
class Home extends React.Component {
  constructor(p) {
    ...
    this.state= {big_screen:true,loc_data:'', loc_ids:[], annotations:[], setAnnotations:[], newAnnotation:[], setNewAnnotation:[], metadata_form:false, metadata:false, metdata_added:false, uploadFile:false, file_info:{}, skip:0, no_more_files:false, annotations_added:false}
  ...
  componentDidMount() {
    ...
    this.resize();
    var data ={'username':this.props.auth.auth.user.username, skip:0, type:"work_files"}
  ...
  metadata( values){
      ...
      var metadata = values
      metadata['username'] = this.props.auth.auth.user.username
      metadata['added_at'] = new Date().toISOString()
      this.setState({metadata: metadata, metdata_added:true})

  }
  next(){
      var skip = this.state.skip
      skip++
      var data ={'username':this.props.auth.auth.user.username, skip:skip, type:"work_files"}
      getNewFile(data).then(function(resp){
  ...
  render() {
    ...
    if (this.props.auth.auth.user.username) {
      return (
        <div ref="myRef">
          <Helmet>
            <title> Audiotation | My Work </title>
          </Helmet>
          <Header/>
          <Container fluid style={{
            ...
            <Row  style={{padding:"20px 0px 30px 0px"}}>
              ...
              {/*  <Button   onClick = {this.metadata_form.bind(this)} color="#ff0000"  size ="sm"  outline   style={{ border:"1px solid rgb(52 0 146)" , borderStyle:"dotted",  paddingTop:"0px  !important", fontSize:"1.2rem !important", borderRadius:"20px"}}>  <span style={{fontSize:"1.2rem", color:"rgb(52 0 146)", padding:"0 20px"}}>  <i className="fa fa-plus-circle " style={{paddingRight:"10px"}}></i> Add Metadata </span>  </Button> */}
               <a href="/myfiles"> <Button    color="#ff0000"  size ="sm"  outline   style={{ border:"1px solid rgb(52 0 146)" , borderStyle:"dotted",  paddingTop:"0px  !important", fontSize:"1.2rem !important", borderRadius:"20px"}}>  <span style={{fontSize:"1.2rem", color:"rgb(52 0 146)", padding:"0 20px"}}>  <i className="fa fa-files-o " style={{paddingRight:"10px"}}></i> My Files </span>  </Button> </a>
    ...
    fontWeight: "500", color: "#888888"}}> Your Annotation Work </span>)}
              ...
              {this.state.metadata_form ?(
                  <Row >
                    ...
                    <div style={{width:"100%", textAlign:"right", marginBottom:"-50px"}}>  <i  onClick = {this.close_metadata_form.bind(this)} className="fa fa-close " style={{paddingRight:"20px" ,color:"rgb(52, 0, 146)", cursor:"pointer"}} />  </div>
                      {this.state.metdata_added ?(<Row  style={{padding:"50px", background: "#fffbfb"}}> Thank you for adding metadata </Row>):(
                          <Metadata   onSubmit={this.metadata.bind(this)}  />
                      )}
